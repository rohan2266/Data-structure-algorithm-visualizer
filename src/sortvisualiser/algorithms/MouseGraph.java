...
public class MouseGraph extends JFrame implements MouseListener, MouseMotionListener, ActionListener {
    ...
    public MouseGraph(boolean render) {
        if (render) {
                        ...
                        public void actionPerformed(ActionEvent ee) {
                            if ((t1.getText() == null) || (t2.getText() == null)) {
                                ...
                                if (path.getLast().size() == 0) {
                                    ...
                                    obj_x.add((double) ((double) vertex.get(obj_to.getLast()).getX()
                                            - (double) vertex.get(obj_from.getLast()).getX()) / (double) 100);
                                    obj_y.add((double) ((double) vertex.get(obj_to.getLast()).getY()
                                            - (double) vertex.get(obj_from.getLast()).getY()) / (double) 100);
    ...
    public void paint(Graphics g) {
        ...
        if (mode == 2) {
            ...
            if (press != null)
                g2.drawLine(press.getX(), press.getY(), curr.getX(), curr.getY());
        }
        ...
        for (Map.Entry<String, LinkedList<Node>> it : graph.entrySet()) {
            ...
            for (Node x : l) {
                g2.setPaint(Color.BLUE);
                g2.drawLine(vertex.get(it.getKey()).getX() + 7, vertex.get(it.getKey()).getY() + 7,
                        vertex.get(x.getName()).getX() + 7, vertex.get(x.getName()).getY() + 7);
                g2.setColor(Color.BLACK);
                g2.drawString(Double.toString(x.getWt()),
                        (float) (vertex.get(it.getKey()).getX() + 7 + vertex.get(x.getName()).getX() + 7) / 2f,
                        (float) (vertex.get(it.getKey()).getY() + 7 + vertex.get(x.getName()).getY() + 7) / 2f);
            }
        }
                    ...
                    public void actionPerformed(ActionEvent ee) {
                        ...
                        for (Map.Entry<String, LinkedList<Node>> it : graph.entrySet()) {
                            ...
                            for (Node x : it.getValue()) {
                                if (!x.getName().equals(v1))
                                    graph1.get(it.getKey()).add(x);
                            }
                            graph = graph1;
                        }
                        repaint();
                        ...
                        try {
                            ...
                            if (name != null) {
                                ...
                                for (Node xx : graph.get(fromm)) {
                                    if (xx.getName().equals(too))
                                        graph.get(fromm).remove(c);
                                    c++;
                                }
                                graph.get(fromm).add(new Node(too, Integer.parseInt(name)));
                                c = 0;
                                for (Node xx : graph.get(too)) {
                                    if (xx.getName().equals(fromm))
                                        graph.get(too).remove(c);
                                    c++;
                                }
    ...
    public void actionPerformed(ActionEvent e) {
        for (int i = 0; i < obj_mode.size(); i++) {
            if (obj_mode.get(i) != -1) {
                ...
                if (Math.abs(obj_currx.get(i).intValue() - vertex.get(obj_to.get(i)).getX()) < 2
                        && Math.abs(obj_curry.get(i).intValue() - vertex.get(obj_to.get(i)).getY()) < 2) {
                    idx.set(i, idx.get(i) + 1);
                    if (idx.get(i) == path.get(i).size() - 1)
                        idx.set(i, 0);
                    ...
                    obj_x.set(i, (double) ((double) vertex.get(obj_to.get(i)).getX()
                            - (double) vertex.get(obj_from.get(i)).getX()) / (double) 100);
                    obj_y.set(i, (double) ((double) vertex.get(obj_to.get(i)).getY()
                            - (double) vertex.get(obj_from.get(i)).getY()) / (double) 100);
                }
            }
        }
        repaint();
    ...
    @Override
    public String getName() { // getname method to return type of sort, used in MainMenuScreen and
        return "Djikstra Algorithm";
    }
